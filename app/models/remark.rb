...
class Remark < ApplicationRecord
  ...
  scope :proposals, -> { where(remark_type: 'proposal').order(created_at: :asc) }
  scope :comments, -> { where(remark_type: 'comment').order(created_at: :asc) }

  after_create_commit lambda {
    broadcast_remove_to [meeting_room, "#{remark_type}s"], target: "#{remark_type}_empty_message"
    broadcast_append_later_to [meeting_room, "#{remark_type}s"], partial: "remarks/#{remark_type}", locals: { remark: self }, target: "#{remark_type}s"
  }
